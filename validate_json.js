...
try {
    ...
    if (e.message.includes('position')) {
        ...
        const lines = fs.readFileSync('JSON_adventures/el-dragon-del-pico-agujahelada.json', 'utf8').substring(0, pos).split('\n');
        ...
        for (let i = Math.max(0, line - 3); i < Math.min(allLines.length, line + 2); i++) {
